...
const empty = {EM_Code:'',EM_Employee_Name:'',DepartmentId:'',DesignationId:'',ContractId:'',CompanyId:'',Contact_No:'',Adhar_No:'',Join_Date:new Date().toISOString().slice(0,10),EM_DOB:'',Releving_Date:'',Address:'',EM_Team:'',EM_Email_ID:'',EM_Rep_Person:'',EM_Status:'Active'}
const SEED = [
  {id:1,...empty,EM_Code:'EMP001',EM_Employee_Name:'RAJESH KUMAR',DepartmentId:'Production',DesignationId:'Engineer',CompanyId:'VELSON INDUSTRIES PVT LTD',Contact_No:'9876543210',Join_Date:'2022-01-15',EM_Status:'Active'},
  {id:2,...empty,EM_Code:'EMP002',EM_Employee_Name:'PRIYA SHARMA',DepartmentId:'Quality',DesignationId:'Inspector',CompanyId:'VELSON INDUSTRIES PVT LTD',Contact_No:'9876543211',Join_Date:'2021-06-01',EM_Status:'Active'},
  {id:3,...empty,EM_Code:'EMP003',EM_Employee_Name:'SURESH BABU',DepartmentId:'Stores',DesignationId:'Supervisor',CompanyId:'VELSON SERVICES LLP',Contact_No:'9876543212',Join_Date:'2020-03-10',EM_Status:'Active'},
]
...
export default function EmployeeMaster(){
  ...
  const validate=()=>{const e={};if(!form.EM_Code.trim())e.EM_Code='Required';if(!form.EM_Employee_Name.trim())e.EM_Employee_Name='Required';if(!form.DepartmentId)e.DepartmentId='Required';setErrors(e);return!Object.keys(e).length}
  const handleSave=()=>{
    if(!validate())return
  ...
  const filtered=rows.filter(r=>[r.EM_Code,r.EM_Employee_Name,r.DepartmentId,r.DesignationId,r.Contact_No,r.EM_Status].some(v=>String(v||'').toLowerCase().includes(search.toLowerCase())))
  ...
  const FI=({label,fk,type='text',req,ro,ml})=>(
    <div className="flex items-center gap-2">
      <label className={`${lbl} w-28 shrink-0`}>{req&&<span className="text-red-500">*</span>}{label} :</label>
      <div className="flex-1"><input type={type} value={form[fk]||''} onChange={e=>sf(fk,e.target.value)} className={ro?`${inp(false)} bg-slate-50`:inp(errors[fk])} readOnly={ro} maxLength={ml}/>{errors[fk]&&<p className="text-[11px] text-red-500 mt-0.5">{errors[fk]}</p>}</div>
    </div>
  )
  const FS=({label,fk,opts,req})=>(
    <div className="flex items-center gap-2">
      <label className={`${lbl} w-28 shrink-0`}>{req&&<span className="text-red-500">*</span>}{label} :</label>
      <div className="flex-1"><select value={form[fk]} onChange={e=>sf(fk,e.target.value)} className={inp(errors[fk])}><option value="">--- Select ---</option>{opts.map(o=><option key={o}>{o}</option>)}</select>{errors[fk]&&<p className="text-[11px] text-red-500 mt-0.5">{errors[fk]}</p>}</div>
    </div>
  )
  ...
  return(
    <div className="p-4 space-y-4 w-full min-w-0">
      ...
      <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-[--color-main] px-4 py-2.5"><h2 className="text-white text-center font-semibold text-[14px]">{editId!==null?'Edit':'Create'} — Employee Master</h2></div>
        <div className="p-4">
          <div className="grid grid-cols-3 gap-x-6 gap-y-3">
            <FI label="Employee Code" fk="EM_Code" req/>
            <div className="col-span-2 flex items-center gap-2"><label className={`${lbl} w-28 shrink-0`}><span className="text-red-500">*</span>Employee Name :</label><div className="flex-1"><input value={form.EM_Employee_Name} onChange={e=>sf('EM_Employee_Name',e.target.value)} className={inp(errors.EM_Employee_Name)}/>{errors.EM_Employee_Name&&<p className="text-[11px] text-red-500 mt-0.5">{errors.EM_Employee_Name}</p>}</div></div>
            ...
            <div className="col-span-3 flex items-start gap-2"><label className={`${lbl} w-28 shrink-0 pt-1`}>Address :</label><textarea value={form.Address} onChange={e=>sf('Address',e.target.value)} rows={3} className="flex-1 border rounded px-2 py-1 text-[13px] border-slate-300 focus:outline-none focus:ring-1 focus:ring-[#0097A7]"/></div>
            <FI label="Team" fk="EM_Team"/>
            <FI label="Email ID" fk="EM_Email_ID" type="email"/>
            <FI label="Reporting Person" fk="EM_Rep_Person"/>
            <FS label="Status" fk="EM_Status" opts={['Active','Relieved']}/>
          </div>
      ...
      <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-[--color-main] px-4 py-2.5"><h2 className="text-white text-center font-semibold text-[14px]">Employee Master Details</h2></div>
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full text-[13px]">
            <thead><tr className="bg-slate-50 border-b border-slate-200">{['S.No','Code','Name','Department','Designation','Contact','Join Date','Status','Edit','Delete','Details'].map(h=><th key={h} className="text-center px-3 py-2.5 font-semibold text-slate-600 text-[12px] uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
            <tbody>
              ...
              :paged.map((r,idx)=>(
                <tr key={r.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${idx%2===1?'bg-slate-50/50':''}`}>
                  ...
                  <td className="px-3 py-2 text-center font-medium">{r.EM_Employee_Name}</td>
                  ...
                  <td className="px-3 py-2 text-center"><span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${r.EM_Status==='Active'?'bg-green-100 text-green-700':'bg-red-100 text-red-600'}`}>{r.EM_Status}</span></td>
      ...
      {detailRow&&<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-2 gap-x-6 gap-y-1.5 max-h-[70vh] overflow-y-auto">
            {[['Code',detailRow.EM_Code],['Name',detailRow.EM_Employee_Name],['Department',detailRow.DepartmentId],['Designation',detailRow.DesignationId],['Contractor',detailRow.ContractId],['Company',detailRow.CompanyId],['Contact',detailRow.Contact_No],['Aadhaar',detailRow.Adhar_No],['Join Date',detailRow.Join_Date],['DOB',detailRow.EM_DOB],['Relieving',detailRow.Releving_Date],['Team',detailRow.EM_Team],['Email',detailRow.EM_Email_ID],['Reporting Person',detailRow.EM_Rep_Person],['Status',detailRow.EM_Status]].map(([l,v])=>(
