...
import { X, Save, ChevronRight, Plus, Trash2, RotateCcw, CreditCard, Landmark, Banknote, Calculator, Receipt, TrendingUp } from 'lucide-react'
...
export default function ReceiptEntry() {
  ...
  const handleSave = () => {
    if (!form.partyName || !form.amount) {
      alert('Required: Party Name and Receipt Amount.')
      return
    }
    setIsSaving(true)
    setTimeout(() => {
      ...
      setIsSaving(false)
      alert('Inward Transaction Recorded Successfully.')
      handleClear()
    }, 800)
  }
    ...
    <div className="bg-[#f4f6f8] min-h-full pb-32">
      <div className="px-6 py-6">
        <div className="flex items-center gap-2 text-[12px] text-slate-400 mb-5 uppercase font-black tracking-tight">
          <span>Dashboard</span> <ChevronRight size={12} /> <span>Account</span> <ChevronRight size={12} /> <span className="text-[#0097A7]">Inward Revenue Entry</span>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[850px]">
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-8 py-5">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-[14px] font-black text-slate-800 uppercase tracking-widest">Inward Transaction Ledger</h2>
            </div>
          ...
          <div className="p-10 space-y-12">
            ...
            <div className="grid grid-cols-12 gap-12 bg-slate-50/50 p-10 rounded-[3rem] border border-slate-100 shadow-inner relative overflow-hidden">
               ...
               <div className="col-span-7 space-y-8 relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-[#0097A7] text-white rounded-2xl flex items-center justify-center font-black">CR</div>
                    <h3 className="text-[13px] font-black text-slate-800 uppercase tracking-widest">Credit Transaction Intake</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                     <div className="space-y-4">
                        <div className="space-y-1">
                           ...
                           <Select options={['RECEIPT', 'PAYMENT']} value={form.receiptType} onChange={u('receiptType')} placeholder="Select Type" />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                           <div className="space-y-1">
                              <Label>Doc Sequence</Label>
                              <Input value={form.receiptNo} onChange={u('receiptNo')} />
                           </div>
                           <div className="space-y-1">
                     ...
                     <div className="space-y-4">
                        <div className="space-y-1">
                           <Label required>Destination Account</Label>
                           <Select options={['CASH IN HAND', 'HDFC MAIN ACCOUNT', 'SBI COLLECTION', 'ICICI REVENUE']} placeholder="Select Gateway" value={form.cashBank} onChange={u('cashBank')} />
                        </div>
                        <div className="space-y-1">
                           <Label required>Client / Debtor Entity</Label>
                           <Input placeholder="Search Client Ledger..." value={form.partyName} onChange={u('partyName')} />
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-12 gap-8 pt-4">
                     <div className="col-span-8 space-y-1">
                        ...
                        <Input placeholder="Internal narration / source of revenue..." value={form.details} onChange={u('details')} />
                     </div>
                     <div className="col-span-4 space-y-1">
                        <Label required>Inward Amount</Label>
                        <Input className="text-right font-black text-[#0097A7] text-lg" placeholder="0.00" value={form.amount} onChange={u('amount')} />
                     </div>
                  </div>
               </div>
               ...
               <div className="col-span-5 bg-white rounded-[2.5rem] p-10 shadow-xl border border-slate-100 space-y-6 relative z-10">
                  <div className="flex items-center justify-between border-b border-slate-50 pb-4">
                    <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                       <Landmark size={16} /> Settlement Protocols
                    </h3>
                  </div>
                  <div className="space-y-4">
                     <div className="space-y-1">
                        <Label>Receiving Institution</Label>
                        <Select options={['SBI - REVENUE', 'HDFC - INWARD', 'ICICI - COLLECTION']} placeholder="Select Bank" value={form.bankName} onChange={u('bankName')} />
                     </div>
                     <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                           ...
                           <Input placeholder="Internal Ref" value={form.extraField} onChange={u('extraField')} />
                        </div>
                     </div>
                     <div className="space-y-1">
                        <Label>Digital / Chq Ref</Label>
                        <Input placeholder="Enter Transaction Ref / Chq No" value={form.chqNo} onChange={u('chqNo')} />
                     </div>
                  </div>
                  <div className="pt-4 flex items-center gap-6">
                    {['Direct-Post', 'Allocation-Hub'].map(mode => (
                 ...
                 <table className="w-full text-left border-collapse min-w-[1700px]">
                   <thead className="bg-[#fcfdfe] text-[10px] uppercase text-slate-400 font-black border-b border-slate-200">
                     <tr>
                       ...
                       <th className="px-6 py-5 border-r border-slate-100">Inward Type</th>
                       <th className="px-6 py-5 border-r border-slate-100">Doc Reference</th>
                       <th className="px-6 py-5 border-r border-slate-100">Doc Date</th>
                       <th className="px-6 py-5 border-r border-slate-100 text-right">Face Value</th>
                       <th className="px-8 py-5 border-r border-slate-100 text-right">Adjusted</th>
                       <th className="px-8 py-5 border-r border-slate-100 text-right">Balance</th>
                       <th className="px-8 py-5 border-r border-slate-100 text-right bg-[#0097A7]/5">Allocated</th>
                       <th className="px-6 py-5 border-r border-slate-100 text-right">Disc/Shift</th>
                       <th className="px-6 py-5 border-r border-slate-100">Allocation Ledger</th>
                   ...
                   <tbody className="divide-y divide-slate-50 text-[13px]">
                     {rows.map((row, idx) => (
                       <tr key={row.id} className="hover:bg-[#0097A7]/5 transition-colors h-14 group">
                         ...
                         <td className="px-2 py-2 border-r border-slate-50"><Input placeholder="Ref" className="!border-none !bg-transparent font-black text-slate-800" value={row.billNo} onChange={e => setRowField(idx, 'billNo', e.target.value)} /></td>
                         ...
                         <td className="px-2 py-2 border-r border-slate-50 bg-[#0097A7]/5"><Input placeholder="0.00" className="!border-none !bg-transparent text-right font-black text-[#0097A7]" value={row.received} onChange={e => setRowField(idx, 'received', e.target.value)} /></td>
                         ...
                         <td className="px-4 py-2 border-r border-slate-50"><Input placeholder="Select Contra Ledger..." className="!border-none !bg-transparent font-black uppercase text-[10px] tracking-tighter" value={row.ledgerAc} onChange={e => setRowField(idx, 'ledgerAc', e.target.value)} /></td>
            ...
            <div className="flex justify-end pt-10">
               ...
               >
                 ...
                 {isSaving ? 'Synchronizing...' : 'Authorize Inward Entry'}
               </button>
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-10 left-[240px] right-10 bg-slate-900 rounded-[3rem] p-8 flex items-center justify-around shadow-2xl z-20 border border-white/10">
        <div className="flex items-center gap-12">
           <div className="flex flex-col">
             <span className="text-white/30 text-[9px] font-black uppercase tracking-widest mb-1">Total Receivable</span>
             <span className="text-[20px] font-black text-white leading-none tracking-tighter">{totalBalanceAmt}</span>
           </div>
           <div className="w-[1px] h-10 bg-white/10" />
           <div className="flex flex-col">
             <span className="text-[#0097A7]/50 text-[9px] font-black uppercase tracking-widest mb-1">Total Received</span>
             <span className="text-[20px] font-black text-[#0097A7] leading-none tracking-tighter">{totalReceivedAmt}</span>
           </div>
        </div>

        <div className="flex items-center gap-12">
           <div className="flex flex-col">
             <span className="text-amber-400/50 text-[9px] font-black uppercase tracking-widest mb-1">Shift / Discount</span>
             <span className="text-[20px] font-black text-amber-500 leading-none tracking-tighter">{totalIntAmt}</span>
           </div>
           <div className="w-[1px] h-10 bg-white/10" />
           <div className="flex flex-col">
             <span className="text-green-400/50 text-[9px] font-black uppercase tracking-widest mb-1">Allocation Sum</span>
             <span className="text-[24px] font-black text-green-500 leading-none tracking-tighter">{grandTotal}</span>
           </div>
        </div>

        <div className="flex flex-col items-end">
           <span className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em] mb-2 italic">Outstanding Liquidity</span>
