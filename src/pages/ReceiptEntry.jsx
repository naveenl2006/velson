  ...
  return (
    <div className="bg-[#f4f6f8] min-h-full pb-20">
      ...
      <div className="px-6 py-6">
        <div className="flex items-center gap-2 text-[12px] text-slate-400 mb-5 uppercase font-bold tracking-tighter">
          <span>Dashboard</span> <ChevronRight size={12} /> <span>Account</span> <ChevronRight size={12} /> <span className="text-[#0097A7]">Receipt Entry</span>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-[13px] font-bold text-slate-700 uppercase">Receipt Details Entry</h2>
            </div>
          ...
          <div className="p-6">
            <div className="grid grid-cols-12 gap-x-12 gap-y-4">
              ...
              <div className="col-span-6 space-y-4">
                ...
                <div className="grid grid-cols-12 items-center gap-4">
                  ...
                  <div className="col-span-4"><Input value={form.receiptNo} onChange={u('receiptNo')} placeholder="R-0000" /></div>
                ...
                <div className="grid grid-cols-12 items-center gap-4">
                  <div className="col-span-3"><Label required>Party</Label></div>
                  <div className="col-span-9"><Input placeholder="Enter Party Name..." value={form.partyName} onChange={u('partyName')} /></div>
                </div>
                ...
                <div className="grid grid-cols-12 items-center gap-4">
                  ...
                  <div className="col-span-3"><Input placeholder="0.00" className="text-right font-bold text-[#0097A7]" value={form.amount} onChange={u('amount')} /></div>
            ...
            <div className="mt-10 border border-slate-200 rounded-xl overflow-hidden shadow-sm">
              <div className="bg-slate-700 px-4 py-2.5 flex items-center justify-between">
                <h3 className="text-white text-[12px] font-bold uppercase tracking-widest">Adjustment Ledger</h3>
                  ...
                  <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 font-bold border-b border-slate-200">
                    <tr>
                      ...
                      {['Bill Type','Bill No','Bill Date','Bill Amt','Adjust Amt','Balance','Received','Shift Amt','Disc Amt','Ledger A/C'].map(h => (
                        <th key={h} className="px-3 py-3 border-r border-slate-200">{h}</th>
                      ))}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rows.map((row, idx) => (
                      <tr key={row.id} className="hover:bg-slate-50 transition-colors group h-10">
                        ...
                        <td className="px-1 py-0.5 border-r border-slate-200"><Input placeholder="Number" className="!border-none !bg-transparent" value={row.billNo} onChange={e => setRowField(idx, 'billNo', e.target.value)} /></td>
                        ...
                        <td className="px-1 py-0.5 border-r border-slate-200"><Input placeholder="0.00" className="!border-none !bg-transparent text-right text-green-600 font-black" value={row.received} onChange={e => setRowField(idx, 'received', e.target.value)} /></td>
                        ...
                        <td className="px-1 py-0.5 border-r border-slate-200"><Input placeholder="Ledger Account..." className="!border-none !bg-transparent font-semibold" value={row.ledgerAc} onChange={e => setRowField(idx, 'ledgerAc', e.target.value)} /></td>
      ...
      <div className="fixed bottom-0 left-[210px] right-0 bg-slate-900 text-white p-4 flex items-center justify-around text-[11px] font-black uppercase tracking-widest z-20 border-t border-white/10">
        <div className="flex flex-col items-center">
          <span className="text-white/40 mb-1">Total Balance</span>
          <span className="text-[16px] text-white">{totalBalanceAmt}</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-white/40 mb-1">Received</span>
          <span className="text-[16px] text-green-400">{totalReceivedAmt}</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-white/40 mb-1">Interest</span>
        ...
        <div className="flex flex-col items-center">
          <span className="text-white/40 mb-1">Remaining</span>
