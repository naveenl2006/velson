...
const empty={VehicleTypeId:'',ServiceDate:'',ServiceDetails:'',NextServiceDate:'',Status:'Active'}
const SEED=[
  {id:1,VehicleTypeId:'Truck',ServiceDate:'2025-01-10',ServiceDetails:'Oil change, filter replacement, brake check',NextServiceDate:'2025-07-10',Status:'Active'},
  {id:2,VehicleTypeId:'Mini Truck',ServiceDate:'2025-03-15',ServiceDetails:'Full service with tyre rotation',NextServiceDate:'2025-09-15',Status:'Active'},
]
...
export default function VehicleServiceMaster(){
  ...
  const validate=()=>{const e={};if(!form.VehicleTypeId)e.VehicleTypeId='Required';if(!form.ServiceDate)e.ServiceDate='Required';setErrors(e);return!Object.keys(e).length}
  const handleSave=()=>{
    if(!validate())return
  ...
  const filtered=rows.filter(r=>[r.VehicleTypeId,r.ServiceDate,r.ServiceDetails,r.Status].some(v=>String(v||'').toLowerCase().includes(search.toLowerCase())))
  ...
  return(
    <div className="p-4 space-y-4 w-full min-w-0">
      ...
      <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-[--color-main] px-4 py-2.5"><h2 className="text-white text-center font-semibold text-[14px]">{editId!==null?'Edit':'Create'} — Vehicle Service Master</h2></div>
        <div className="p-4">
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 max-w-3xl">
            <div className="flex items-center gap-2"><label className={`${lbl} w-32 shrink-0`}><span className="text-red-500">*</span>Vehicle Type :</label><div className="flex-1"><select value={form.VehicleTypeId} onChange={e=>sf('VehicleTypeId',e.target.value)} className={inp(errors.VehicleTypeId)}><option value="">--- Select ---</option>{VEHICLE_TYPES.map(v=><option key={v}>{v}</option>)}</select>{errors.VehicleTypeId&&<p className="text-[11px] text-red-500 mt-0.5">{errors.VehicleTypeId}</p>}</div></div>
            <div className="flex items-center gap-2"><label className={`${lbl} w-32 shrink-0`}><span className="text-red-500">*</span>Service Date :</label><div className="flex-1"><input type="date" value={form.ServiceDate} onChange={e=>sf('ServiceDate',e.target.value)} className={inp(errors.ServiceDate)}/>{errors.ServiceDate&&<p className="text-[11px] text-red-500 mt-0.5">{errors.ServiceDate}</p>}</div></div>
            <div className="col-span-2 flex items-start gap-2"><label className={`${lbl} w-32 shrink-0 pt-1`}>Service Details :</label><textarea value={form.ServiceDetails} onChange={e=>sf('ServiceDetails',e.target.value)} rows={3} className="flex-1 border rounded px-2 py-1 text-[13px] border-slate-300 focus:outline-none focus:ring-1 focus:ring-[#0097A7]"/></div>
            <div className="flex items-center gap-2"><label className={`${lbl} w-32 shrink-0`}>Next Service :</label><input type="date" value={form.NextServiceDate} onChange={e=>sf('NextServiceDate',e.target.value)} className={inp(false)}/></div>
      ...
      <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-[--color-main] px-4 py-2.5"><h2 className="text-white text-center font-semibold text-[14px]">Vehicle Service Master Details</h2></div>
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full text-[13px]">
            <thead><tr className="bg-slate-50 border-b border-slate-200">{['S.No','Vehicle Type','Service Date','Next Service','Status','Edit','Delete','Details'].map(h=><th key={h} className="text-center px-3 py-2.5 font-semibold text-slate-600 text-[12px] uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
            <tbody>
              ...
              :paged.map((r,idx)=>(
                <tr key={r.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${idx%2===1?'bg-slate-50/50':''}`}>
                  ...
                  <td className="px-3 py-2 text-center font-medium">{r.VehicleTypeId}</td>
                  <td className="px-3 py-2 text-center">{r.ServiceDate}</td>
                  <td className="px-3 py-2 text-center">{r.NextServiceDate||'—'}</td>
      ...
      {detailRow&&<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
          <div className="bg-gradient-to-r from-[#0097A7] to-[#00BCD4] px-6 py-4 flex items-center justify-between"><h2 className="text-white font-bold text-[15px]">Vehicle Service Details</h2><button onClick={()=>setDetailRow(null)} className="text-white/80 hover:text-white"><X className="w-5 h-5"/></button></div>
          <div className="p-5 space-y-1.5">
            {[['Vehicle Type',detailRow.VehicleTypeId],['Service Date',detailRow.ServiceDate],['Next Service',detailRow.NextServiceDate],['Status',detailRow.Status],['Service Details',detailRow.ServiceDetails]].map(([l,v])=>(
