...
import { 
  ...
  X, Download, FileSpreadsheet, FileJson, Filter, Settings, Database, ArrowRight, Eye, CreditCard
} from 'lucide-react'
...
const SEED_DATA = [
  { id: 1, receiptNo: 'P-1001', date: '2026-04-10', receiptType: 'PAYMENT', partyName: 'RELIANCE INDUSTRIES', amount: '25000.00', cashBank: 'BANK', bankName: 'SBI', createdBy: 'Admin' },
  { id: 2, receiptNo: 'P-1002', date: '2026-04-12', receiptType: 'PAYMENT', partyName: 'TATA STEEL', amount: '15000.00', cashBank: 'BANK', bankName: 'HDFC', createdBy: 'Admin' },
  { id: 3, receiptNo: 'P-1003', date: '2026-04-14', receiptType: 'PAYMENT', partyName: 'MAHINDRA LOGISTICS', amount: '8000.00', cashBank: 'CASH', bankName: '-', createdBy: 'Admin' },
  { id: 4, receiptNo: 'P-1004', date: '2026-04-15', receiptType: 'PAYMENT', partyName: 'BLUE DART', amount: '45000.00', cashBank: 'BANK', bankName: 'ICICI', createdBy: 'Admin' },
]
...
export default function PaymentDetails() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('velson_payments')
    if (saved) {
      ...
      localStorage.setItem('velson_payments', JSON.stringify(SEED_DATA))
    }
  }, [])
  ...
  const handleDelete = () => {
    ...
    localStorage.setItem('velson_payments', JSON.stringify(next))
    ...
    <div className="bg-[#f4f6f8] min-h-full pb-32">
      ...
      <div className="px-6 py-6">
        <div className="flex items-center gap-2 text-[12px] text-slate-400 mb-5 uppercase font-black tracking-tight">
          <span>Dashboard</span> <ChevronRight size={12} /> <span>Account</span> <ChevronRight size={12} /> <span className="text-[#0097A7]">Disbursement Archive</span>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[900px]">
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-10 py-6">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-[16px] font-black text-slate-800 uppercase tracking-[0.2em]">Financial Disbursement Archive</h2>
            </div>
            <div className="flex gap-4">
              <button 
                onClick={() => selectedId ? setConfirmDelete({ open: true, id: selectedId }) : alert('Select an entry first')}
            ...
            <div className="bg-slate-50/50 p-10 rounded-[3.5rem] border border-slate-100 shadow-inner flex items-center justify-between relative overflow-hidden">
               ...
               <div className="flex items-center gap-8 relative z-10">
                  <div className="flex items-center gap-4 border-l border-slate-200 pl-8">
                    {[{ icon: <FileSpreadsheet size={16} />, l: 'EXCEL' }, { icon: <FileJson size={16} />, l: 'PAY.PDF' }].map(tool => (
                      <button key={tool.l} className="flex items-center gap-2 text-slate-300 hover:text-rose-600 text-[10px] font-black uppercase tracking-widest transition-all italic">
                        {tool.icon} {tool.l}
                      </button>
                    ))}
                  </div>
                 ...
                 <table className="w-full text-left border-collapse min-w-[1400px]">
                   <thead className="bg-[#fcfdfe] text-[10px] uppercase text-slate-400 font-black border-b border-slate-200">
                     <tr>
                       ...
                       <th className="px-8 py-6 border-r border-slate-100">Disbursement Ref</th>
                       ...
                       <th className="px-8 py-6 border-r border-slate-100">Ledger Recipient</th>
                       <th className="px-8 py-6 border-r border-slate-100 text-right w-40">Payment Value</th>
                     ...
                     {filteredData.length === 0 ? (
                       <tr>
                         <td colSpan={9} className="py-24 text-center text-slate-200 italic font-black uppercase tracking-widest opacity-30">
                            No disbursement records discovered in the current temporal frame.
                         </td>
                       </tr>
                     ) : (
                       filteredData.map((row, idx) => (
                         <tr 
                           ...
                           className={`cursor-pointer transition-all h-18 group ${selectedId === row.id ? 'bg-rose-50/20' : 'hover:bg-slate-50'}`}
                         >
                           ...
                           <td className="px-8 py-2 border-r border-slate-50 text-right font-black text-rose-600 text-lg tracking-tighter">{parseFloat(row.amount).toFixed(2)}</td>
                           ...
                           <td className="px-8 py-2 text-center">
                              <button className="p-2.5 text-slate-200 hover:text-rose-600 hover:bg-rose-100 rounded-xl transition-all">
                                 <Eye size={18} />
                              </button>
                           </td>
                         </tr>
            ...
            <div className="mt-12 bg-slate-900 rounded-[3.5rem] p-12 flex items-center justify-between shadow-2xl relative overflow-hidden border border-slate-800">
               ...
               <div className="flex items-center gap-24 relative z-10">
                  <div className="flex flex-col">
                    <p className="text-white/30 text-[10px] font-black uppercase tracking-[0.5em] mb-4 italic">Total Outflow Summary</p>
                    <div className="flex items-center gap-12">
                       <div>
                          <p className="text-slate-400 text-[10px] font-black uppercase mb-1">Total Entries</p>
                          <p className="text-[28px] font-black text-white leading-none tracking-tighter">{filteredData.length}</p>
                       </div>
                       <div className="w-[1px] h-10 bg-white/10" />
                       <div className="flex flex-col">
                          <p className="text-rose-500 text-[10px] font-black uppercase mb-1">Aggregate Disbursement</p>
               ...
               <div className="text-right relative z-10 opacity-30 flex flex-col items-end gap-3">
                  <div className="flex items-center gap-4">
                    <span className="text-white text-[11px] font-black uppercase tracking-[0.4em] italic text-right leading-tight text-white/50">Disbursement Integrity Core</span>
