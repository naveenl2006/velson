...
const empty={IC_vCode:'',IC_vName:'',IC_vDescription:'',IC_cStatus:'A'}
const SEED=[
  {id:1,IC_vCode:'SF',IC_vName:'Surface Finish',IC_vDescription:'Checks surface roughness and finish quality against Ra specification.',IC_cStatus:'A'},
  {id:2,IC_vCode:'HRD',IC_vName:'Hardness',IC_vDescription:'Verifies material hardness in HRC/HRB scale per material spec.',IC_cStatus:'A'},
  {id:3,IC_vCode:'RND',IC_vName:'Roundness',IC_vDescription:'Measures circularity/roundness deviation of cylindrical surfaces.',IC_cStatus:'A'},
  {id:4,IC_vCode:'THK',IC_vName:'Thickness',IC_vDescription:'Wall thickness measurement using ultrasonic or digital gauge.',IC_cStatus:'I'},
]
...
export default function QCInspectionChar(){
  ...
  const validate=()=>{const e={};if(!form.IC_vCode.trim())e.IC_vCode='Required';if(!form.IC_vName.trim())e.IC_vName='Required';setErrors(e);return!Object.keys(e).length}
  const handleSave=()=>{
    if(!validate())return
  ...
  const filtered=rows.filter(r=>[r.IC_vCode,r.IC_vName,r.IC_vDescription].some(v=>String(v||'').toLowerCase().includes(search.toLowerCase())))
  ...
  return(
    <div className="p-4 space-y-4 w-full min-w-0">
      <div className="flex items-center gap-2 text-[12px] text-slate-400">
        ...
        <span className="text-[#0097A7] font-semibold">QC Inspection Character Master</span>
      </div>
      <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-[--color-main] px-4 py-2.5"><h2 className="text-white text-center font-semibold text-[14px]">{editId!==null?'Edit':'Create'} — QC Inspection Character Master</h2></div>
        <div className="p-4">
          <div className="max-w-xl space-y-3">
            <div className="flex items-center gap-2"><label className={`${lbl} w-28 shrink-0`}><span className="text-red-500">*</span>Char. Code :</label><div className="flex-1"><input value={form.IC_vCode} onChange={e=>sf('IC_vCode',e.target.value.toUpperCase())} className={inp(errors.IC_vCode)} placeholder="e.g. SF, HRD"/>{errors.IC_vCode&&<p className="text-[11px] text-red-500 mt-0.5">{errors.IC_vCode}</p>}</div></div>
            <div className="flex items-center gap-2"><label className={`${lbl} w-28 shrink-0`}><span className="text-red-500">*</span>Char. Name :</label><div className="flex-1"><input value={form.IC_vName} onChange={e=>sf('IC_vName',e.target.value)} className={inp(errors.IC_vName)} placeholder="e.g. Surface Finish"/>{errors.IC_vName&&<p className="text-[11px] text-red-500 mt-0.5">{errors.IC_vName}</p>}</div></div>
            <div className="flex items-start gap-2"><label className={`${lbl} w-28 shrink-0 pt-1`}>Description :</label><textarea value={form.IC_vDescription} onChange={e=>sf('IC_vDescription',e.target.value)} rows={3} className="flex-1 border rounded px-2 py-1 text-[13px] border-slate-300 focus:outline-none focus:ring-1 focus:ring-[#0097A7]"/></div>
            <div className="flex items-center gap-2"><label className={`${lbl} w-28 shrink-0`}>Status :</label><select value={form.IC_cStatus} onChange={e=>sf('IC_cStatus',e.target.value)} className={`w-40 ${inp(false)}`}><option value="A">A — Active</option><option value="I">I — Inactive</option></select></div>
          </div>
      ...
      <div className="bg-white rounded border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-[--color-main] px-4 py-2.5"><h2 className="text-white text-center font-semibold text-[14px]">QC Inspection Character Details</h2></div>
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full text-[13px]">
            <thead><tr className="bg-slate-50 border-b border-slate-200">{['S.No','Code','Characteristic Name','Description','Status','Edit','Delete','Details'].map(h=><th key={h} className="text-center px-3 py-2.5 font-semibold text-slate-600 text-[12px] uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
            <tbody>
              ...
              :paged.map((r,idx)=>(
                <tr key={r.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${idx%2===1?'bg-slate-50/50':''}`}>
                  ...
                  <td className="px-3 py-2 text-center font-mono font-bold text-[#0097A7]">{r.IC_vCode}</td>
                  <td className="px-3 py-2 text-center font-medium">{r.IC_vName}</td>
                  <td className="px-3 py-2 text-left max-w-xs truncate">{r.IC_vDescription}</td>
                  <td className="px-3 py-2 text-center"><span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${r.IC_cStatus==='A'?'bg-green-100 text-green-700':'bg-slate-100 text-slate-500'}`}>{r.IC_cStatus==='A'?'Active':'Inactive'}</span></td>
      ...
      {detailRow&&<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
          <div className="bg-gradient-to-r from-[#0097A7] to-[#00BCD4] px-6 py-4 flex items-center justify-between"><h2 className="text-white font-bold text-[15px]">Inspection Character Details</h2><button onClick={()=>setDetailRow(null)} className="text-white/80 hover:text-white"><X className="w-5 h-5"/></button></div>
          <div className="p-5 space-y-1.5">
            {[['Code',detailRow.IC_vCode],['Name',detailRow.IC_vName],['Status',detailRow.IC_cStatus==='A'?'Active':'Inactive'],['Description',detailRow.IC_vDescription]].map(([l,v])=>(
